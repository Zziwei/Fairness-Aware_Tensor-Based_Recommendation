...
def FATR_Newton(y, features, feature_d, feature_n, r=20, omega=None, omega_groups=None, reg_para=0.1,
                Freg_para=0.1, tol=1e-4, maxiter=500, init='random', printitn=100):
    ...
    #     feature_centered = features - numpy.mean(features, axis=0)
    ...
    B = U[feature_d]
    B = B[:, :len(B[:]) - feature_vn]
    B2tB1 = np.dot(features.T, B)
    B2B1Fnorm = numpy.linalg.norm(B2tB1)
    print 'Initial: B1B2F={0}'.format(B2B1Fnorm)
    ...
    for iter in range(1, maxiters + 1):
        ...
        for n in range(ndimsX):
            ...
            if n == feature_d:
                B_old_part1 = A[:, :len(A[0]) - feature_vn]
                fft = np.dot(features, features.T)
                hessian = fft * reg_para
                gradient = reg_para * np.dot(fft, B_old_part1)

                Bnew_part1 = B_old_part1 - np.dot(np.linalg.pinv(hessian), gradient)
                A = np.concatenate((Bnew_part1, features), axis=1)
            U[n] = A
        ...
        B2tB1 = np.dot(features.T, Bnew_part1)
        B2B1Fnorm = numpy.linalg.norm(B2tB1)
